"""
Taken from https://github.com/beabevi/ESAN and possibly slightly adapted
"""
...
class DSSnetwork(torch.nn.Module):
    ...
    def forward(self, batched_data):
        ...
        for i in range(len(self.gnn_list)):
            ...
            node_embs.append(torch_scatter.scatter(src=subgraph_pool(x, batched_data, global_mean_pool), index=batched_data.subgraph_idx_batch, dim=0, reduce="mean"))
                # print(f"x_sum_{i}: ", x_sum.shape)
        ...
        return self.final_layers(h_graph), h_graph, node_embs
