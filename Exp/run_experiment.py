...
def get_directory(args):
    ...
    """
    return os.path.join(config.RESULTS_PATH, f"{args.dataset}_{args.x}_{os.path.split(args.grid_file)[-1]}") 
...
def parse_args(passed_args):
    ...
    parser.add_argument('--x', type=int, default=-1,
                    help='A value for the x parameter in run_model (default: 0)')
...
def run_final_evaluation(args, final_eval_path, best_params):
    ...
    for iteration in range(nr_tries):
        ...
        i = 0
        output_path = os.path.join(final_eval_path, f"eval_{i}.json")
